...
function WidgetsPopup({ onClose }) {
  const { widgets } = useArchiveContent();
  ...
  return (
    <ArchivePopupShell
      node="ARCHIVE NODE 04"
      title="HTML WIDGET VAULT"
      subtitle="공유 가능한 HTML 조각을 미리 보고, 필요한 코드를 복사합니다."
      meta={`WIDGET_INDEX: ${widgets.length}`}
      onClose={onClose}
    >
